...
int main(int argc, char *argv[])
{
    ...
    cout << path_to_project_dir << '\n';

    TCHAR path_to_protect_file[BUFSIZE];
    sprintf(path_to_protect_file, "%s\\%s", path_to_project_dir, PROTECT_FILENAME);

    /* Пока закомментил, это часть, где я экспериментировал с отображением, если захочешь - потрогаешь
     * https://eax.me/winapi-file-mapping/ - здесь объяснение, плюс рядом есть ссылка на полный код
    // в mmap - структура, содержащая:
    // HANDLE файла hFile, позволяющий работать с отображением как с файлом
    // HANDLE для самого map hMapping
    // Количество данных в отображении по байтам fsize
    // Буфер с данными из отображения в виде буфера char*.
    ...
    // Демонстрация работы: в данном случае я использую getline для потока строк и вывожу первые 2 строки
    // т.е. можно спокойно заменить ввод из файла на ввод из строкового потока
    stringstream s;
    s << mmap->dataPtr;
    char buf[BUFSIZE];
    s.getline(buf, BUFSIZE);
    cout << buf << '\n';
    s.getline(buf, BUFSIZE);
    cout << buf << '\n';
